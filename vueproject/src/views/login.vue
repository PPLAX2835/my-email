    ...
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-row>
        ...
        <el-col :span="12" class="code-box">
          <img :src="codeimg" style="width:100%;" alt="" class="codeimg" @click="getcode()">
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      // 记住密码
      rememberpwd: false,
      codeimg: '/api/auth/captchaimg', //这里配置验证码接口
      ruleForm: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.logining = true
          // 深拷贝表单对象
          let loginForm = JSON.parse(JSON.stringify(this.ruleForm))
          loginForm.code = padStringWithRandom(loginForm.code);
          let iv = padStringWithRandom("LaxHateTheWorld.")
          // 加密s
          loginForm.password = encrypt(loginForm.password, loginForm.code, iv);
          // 调用登录接口
