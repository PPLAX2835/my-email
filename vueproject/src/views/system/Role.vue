  ...
  <div>
    ...
    <el-form :inline="true" :model="formInline" class="user-search">
      ...
      <el-form-item>
        ...
        <el-button size="small" type="danger" icon="el-icon-delete" @click="deleteRole()">删除所选项</el-button>
      </el-form-item>
    </el-form>
    <!--列表-->
    <el-table size="small" :data="listData" @selection-change="selectChange" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column sortable prop="rid" label="rid" width="60">
      </el-table-column>
      <el-table-column sortable prop="roleName" label="角色名称" width="150">
      </el-table-column>
      <el-table-column sortable prop="roleNo" label="角色代码" width="150">
      </el-table-column>
      <el-table-column sortable prop="roleDescription" label="角色描述" width="300">
      </el-table-column>
      <el-table-column sortable prop="permissionLevel" label="权限等级" width="60">
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click='closeDialog("edit")'>
      <el-form label-width="120px" :model="editForm" ref="editForm" :rules="rules">
        ...
        <el-form-item label="角色描述" prop="roleDescription">
           <el-input size="small" v-model="editForm.roleDescription" type="textarea" :rows="2" auto-complete="off" placeholder="请输入角色描述" ></el-input>
        </el-form-item>
        <el-form-item label="权限等级" prop="permissionLevel">
            <el-slider v-model="editForm.permissionLevel" max="10"></el-slider>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleEdit: function(index, row) {
      ...
      if (row != undefined && row != 'undefined') {
        ...
        this.editForm.permissionLevel = row.permissionLevel
        this.editForm.roleDescription = row.roleDescription
        ...
        .then(() => {
          if (!(index === undefined) && !(row === undefined)) {
            ...
            for (let i = 0; i < this.selectdata.ids.length; i++){
              ids.push(this.selectdata.ids[i].rid);
            }
            console.log(ids)
            // 调用接口
            roleDelete(ids)
    ...
    menuAccess: function(index, row) {
      ...
      this.saverid = row.rid
      RoleRightTree(row.rid)
        .then(res => {
          if (res.data.success) {
            this.$message({
              type: 'success',
